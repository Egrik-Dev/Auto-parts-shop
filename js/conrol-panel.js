...
let currentActiveTagElement = document.querySelector(`.control-panel__tags-item--checked`);

tagsItemElements.forEach((item) => {
  item.addEventListener(`click`, (evt) => {
    evt.preventDefault();
    if (currentActiveTagElement) {
      currentActiveTagElement.classList.remove(`control-panel__tags-item--checked`);
    }
